...
export function ActaConfigServer({ children }: { children: ReactNode }) {
  ...
  const testnetApiKey =
    process.env.ACTA_API_KEY_TESTNET?.trim() || process.env.ACTA_API_KEY?.trim() || '';

  // Debug: Log in server (only visible in server console)
  if (process.env.NODE_ENV === 'development') {
    console.log('[ActaConfigServer] Testnet API Key:', {
      testnet: testnetApiKey ? `${testnetApiKey.substring(0, 10)}...` : 'missing',
    });
  }
...
# ACTA_API_KEY=your-api-key`}
  ...
  return <ActaConfigProvider testnetApiKey={testnetApiKey}>{children}</ActaConfigProvider>;
}
